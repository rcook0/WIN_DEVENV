param([string] $ListPath = "./repos.txt")
...
Get-Content $ListPath | ForEach-Object {
  ...
  $parts = $_.Split(";",2); $url = $parts[0].Trim(); $dst = $parts[1].Trim()
  Ensure-Dir (Split-Path -Parent $dst)
  if (Test-Path (Join-Path $dst ".git")) { git -C $dst pull --rebase --autostash }
  elseif (Test-Path $dst) { Write-Host "→ Exists but not a git repo: $dst" }
