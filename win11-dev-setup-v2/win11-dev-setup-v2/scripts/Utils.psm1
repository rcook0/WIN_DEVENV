function Test-Admin {
  ...
  if (-not $isAdmin) {
    ...
    else { Write-Error "Please run PowerShell as Administrator." }
  }
}
...
function Exec {
  ...
  if ($p.ExitCode -ne $SuccessCode) { throw "Command failed with exit code $($p.ExitCode): $Cmd" }
}

function Try-Exec { param([string]$Cmd) try { Exec -Cmd $Cmd; return $true } catch { Write-Warning $_; return $false } }

function Ensure-Path { param([string]$PathToAdd)
